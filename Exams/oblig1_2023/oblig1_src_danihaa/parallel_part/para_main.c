...
int main(int nargs, char **args)
{
  ...
  double openmp_start = omp_get_wtime(); // clock() cannot be used because it would give the sum of the cpu times of all threads
  for (int l = 0; l < timing_repeats; l++)
  {
  ...
  double time_spent = (openmp_end - openmp_start) / timing_repeats;
  printf("AVG. Time omp_regular: %f \n", time_spent);

  printf("nx=%d, ny=%d, T=%g, dt=%g, error=%e\n", nx, ny, t, dt,
         omp_compute_numerical_error(nx, ny, dx, dy, t, u)); // only needs to be done once because will always be the same
  ...
  time_spent = (openmp_end - openmp_start) / timing_repeats;
  printf("Time spent omp_fast: %f \n", time_spent);
  printf("nx=%d, ny=%d, T=%g, dt=%g, error=%e\n", nx, ny, t, dt,
